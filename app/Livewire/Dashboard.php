...
class Dashboard extends Component
{
    ...
    public $productImportCsv;
    public array $importTargetShopIds = [];
    ...
    public function closeImportModal(): void
    {
        ...
        $this->productImportCsv = null;
    }

    public function selectAllImportStores(): void
    {
        $this->importTargetShopIds = Shop::pluck('id')->map(fn($id) => (string)$id)->toArray();
    }

    public function clearImportStores(): void
    {
        $this->importTargetShopIds = [];
    }

    /**
     * Import products from CSV using single/multi-store rules engine.
     */
    public function importProductsCsv(): void
    {
        $this->validate([
            'productImportCsv' => 'required|file|mimes:csv,txt|max:51200',
        ]);

        $path = $this->productImportCsv->store('imports');

        $singleShopId = count($this->importTargetShopIds) === 1 ? (int)$this->importTargetShopIds[0] : null;
        ...
        // Launch background process using nohup so web request completes in < 100ms with zero 504 Timeouts!
        $phpBinary = file_exists('/opt/alt/php84/usr/bin/php') ? '/opt/alt/php84/usr/bin/php' : 'php';
        $artisan = base_path('artisan');
        $targetShopsArg = !empty($this->importTargetShopIds) ? " --target-shops=" . implode(',', $this->importTargetShopIds) : "";

        $cmd = "nohup {$phpBinary} {$artisan} shopify:process-sync-job {$syncJob->id}{$targetShopsArg} > /dev/null 2>&1 &";
        @exec($cmd);

        if (config('queue.default') !== 'sync') {
            \App\Jobs\BulkImportCsvJob::dispatch($syncJob, $this->importTargetShopIds);
        }

        $storeCount = count($this->importTargetShopIds);
        $targetMsg = $storeCount > 0 ? "targeted to {$storeCount} selected store(s)" : "targeting all connected stores";

        session()->flash('success', "Product CSV Import started asynchronously in background ({$targetMsg}). Job #{$syncJob->id}");
        $this->productImportCsv = null;
        $this->showImportModal = false;
    }
