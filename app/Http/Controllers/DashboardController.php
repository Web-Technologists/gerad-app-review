...
class DashboardController extends Controller
{
    ...
    public function importCsv(Request $request)
    {
        ...
        // Launch background process using nohup so web response returns in < 100ms with zero 504 Timeouts!
        $phpBinary = file_exists('/opt/alt/php84/usr/bin/php') ? '/opt/alt/php84/usr/bin/php' : 'php';
        $artisan = base_path('artisan');
        $shopArg = $request->input('shop_id') ? " --target-shops=" . $request->input('shop_id') : "";

        $cmd = "nohup {$phpBinary} {$artisan} shopify:process-sync-job {$syncJob->id}{$shopArg} > /dev/null 2>&1 &";
        @exec($cmd);
        ...
        return redirect()->back()->with('success', "CSV Import started asynchronously in background. Job #{$syncJob->id}");
    }
    ...
    public function jobStatus($job_id)
    {
        ...
        $downloadUrl = null;
        if ($job->status === 'completed' && in_array($job->type, ['csv_export', 'licensing_export', 'csv_import']) && $job->file_path) {
    ...
    /**
     * Download completed export file or audit log CSV.
    ...
    public function downloadExport($job_id)
    {
        ...
        if (!in_array($job->type, ['csv_export', 'licensing_export', 'csv_import']) || $job->status !== 'completed' || !$job->file_path) {
            abort(404, 'File not available.');
        }

        if (!Storage::exists($job->file_path)) {
            abort(404, 'Export / Audit file expired or not found.');
        }
